...
class NetCSFlow(nn.Module):
    def __init__(self, args):
        ...
        self.map_size = (
            self.args.dataset.image_size // 12,
            self.args.dataset.image_size // 12,
        )
        self.kernel_sizes = [3] * (self.args.model.n_coupling_blocks - 1) + [5]
        self.density_estimator = self.get_cs_flow_model(
            input_dim=self.args.model.n_feat
        )

    def get_cs_flow_model(self, input_dim):
        nodes = list()
        nodes.append(
            InputNode(input_dim, self.map_size[0], self.map_size[1], name="input")
        )
        nodes.append(
            InputNode(
                input_dim, self.map_size[0] // 2, self.map_size[1] // 2, name="input2"
            )
        )
        nodes.append(
            InputNode(
                input_dim, self.map_size[0] // 4, self.map_size[1] // 4, name="input3"
            )
        )
        ...
        for k in range(self.args.model.n_coupling_blocks):
            ...
            nodes.append(
                Node(node_to_permute, ParallelPermute, {"seed": k}, name=f"permute_{k}")
            )
            nodes.append(
                Node(
                    ...
                    {
                        "clamp": self.args.model.clamp,
                        "F_class": CrossConvolutions,
                        "F_args": {
                            "channels_hidden": self.args.model.fc_internal,
                            "kernel_size": self.kernel_sizes[k],
                            "block_no": k,
                        },
                    },
                    name=f"fc1_{k}",
    ...
    def eff_ext(self, x, use_layer=36):
        x = self.feature_extractor._swish(
            self.feature_extractor._bn0(self.feature_extractor._conv_stem(x))
        )
        # Blocks
        for idx, block in enumerate(self.feature_extractor._blocks):
            ...
            if drop_connect_rate:
                drop_connect_rate *= float(idx) / len(
                    self.feature_extractor._blocks
        ...
        for s in range(self.args.model.n_scales):
            feat_s = (
                F.interpolate(
                    x,
                    size=(
                        self.args.dataset.image_size // (2**s),
                        self.args.dataset.image_size // (2**s),
                    ),
                )
                if s > 0
    ...
    def forward_logits(self, y):
        z, log_jac_det = self.density_estimator(y), self.density_estimator.jacobian(
            run_forward=False
        )
