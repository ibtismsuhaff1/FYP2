...
def csflow_eval(args, epoch, dataloaders_test, learned_tasks, net):
    ...
    task_num = 0
    for idx, (dataloader_test, learned_task) in enumerate(
        zip(dataloaders_test, learned_tasks)
    ):
    ...
    if args.eval.visualization:
        ...
        compare_histogram(
            np.array(eval_task_wise_scores_np),
            np.array(eval_task_wise_labels_np),
            start=0,
            thresh=5,
            interval=1,
            name=name,
    ...
    if args.model.method == "csflow":
        ...
        for idx, (dataloader_test, learned_task) in enumerate(
            ...
            if args.eval.visualization:
                ...
                plot_tsne(
                    labels, np.array(embeds), defect_name=name, save_path=tnse_save_path
                )
...
if __name__ == "__main__":
    ...
    net, density = torch.load(f"{args.save_path}/net.pth"), torch.load(
        f"{args.save_path}/density.pth"
    )
